...
def main():
    ...
    with model:
        ...
        vocab.add("Sum", np.add(stored_value_1, stored_value_2))

        # Define the control signal inputs as random vectors
        #r1 = np.random.rand(num_dimensions) - np.array([0.5]*num_dimensions)
        r1 = [1] * num_dimensions
        r1 = r1 / np.linalg.norm(r1)
        #r2 = np.random.rand(num_dimensions) - np.array([0.5]*num_dimensions)
        ...
        # Control system
        actions = spa.Actions('dot(word_buffer1, Stored_value_2) --> word_buffer3=Stored_value_2', 'dot(word_buffer1, Stored_value_1) --> word_buffer3=Stored_value_1+Stored_value_2')
        
        #actions = spa.Actions('dot(control, Start_signal) --> word_buffer3 = word_buffer1 + word_buffer2')
